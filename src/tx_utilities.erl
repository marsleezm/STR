...
%%
%% -------------------------------------------------------------------
%% @doc This file implements functions that will be used for creating
...
-export([create_tx_id/1, 
         create_tx_id/2, 
         create_tx_id/3,
         now_microsec/0,
         open_table/2,
         open_public_table/1,
         open_private_table/1,
         get_table_name/2]).
...
%% @doc: Return the current time (up to micro second) in the format of an integer. 
now_microsec() ->
